...
class TestDefaultValues:
    ...
    def test_forkhub_settings_all_defaults(self) -> None:
        ...
        assert settings.anthropic.analysis_budget_usd == 0.50
        assert settings.anthropic.model == "sonnet"
        assert settings.anthropic.digest_model == "haiku"
        # Database
        assert settings.database.path == "~/.local/share/forkhub/forkhub.db"
        # Sync
        assert settings.sync.polling_interval == "6h"
        assert settings.sync.max_forks_per_repo == 5000
        assert settings.sync.max_github_requests_per_hour == 4000
        # Analysis
        ...
        assert settings.digest.day_of_week == "monday"
        assert settings.digest.time == "09:00"
        assert settings.digest.min_significance == 5
        assert settings.digest.backends == ["console"]
        # Tracking
        assert settings.tracking.default_fork_depth == 1
        assert settings.tracking.auto_discover_owned is True
        assert settings.tracking.track_sibling_forks is True
...
"""
    ...
    def test_toml_loads_all_sections(self, toml_file: Path) -> None:
